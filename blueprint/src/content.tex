...
\begin{definition}[Rotated roots]\label{def:rotated_roots}
\lean{rotatedRootsOfUnity, realProjections, realProjectionsList}
\leanok
When the $N$-th roots of unity are rotated by an angle $\varphi$ (equivalently, multiplied by $\e^{i\varphi}$) and projected onto the real axis, we obtain the $N$ real numbers
\begin{equation}
r_k(\varphi) = \cos\left(\varphi + \frac{2\pi k}{N}\right), \quad k = 0, 1, \ldots, N-1.
\end{equation}
The rotation angle $\varphi$ controls the orientation of the $N$-gon, while the projection yields a multiset of real values in $[-1, 1]$.
\lean{realProjection_eq_cos, realProjection_mem_Icc, rotatedRoot_eq_exp, card_realProjectionsList}
\leanok
\end{definition}
...
\begin{definition}[Unscaled polynomial]\label{def:unscaled_poly}
\lean{polynomialFromRealRoots, unscaledPolynomial}
\leanok
\uses{def:rotated_roots}
Let $P_N(x; \varphi)$ denote the monic polynomial having the rotated roots as roots:
\begin{equation}
P_N(x; \varphi) = \prod_{k=0}^{N-1} \left(x - r_k(\varphi)\right).
\end{equation}
Since the roots are real and lie in $[-1, 1]$, this polynomial has real coefficients and degree $N$, and each $r_k(\varphi)$ is a root.
\lean{polynomialFromRealRoots_degree, unscaledPolynomial_degree, unscaledPolynomial_monic, polynomialFromRealRoots_eval_mem, realProjection_mem_list, unscaledPolynomial_eval_at_projection}
\leanok
\end{definition}

\begin{definition}[Scaled polynomial]\label{def:scaled_poly}
\lean{scaledPolynomial}
\leanok
\uses{def:unscaled_poly}
...
\leanok
\uses{lem:power_sum_invariance, thm:newton_identities, lem:chebyshev_roots, def:scaled_poly}
...
\lean{ChebyshevCircles.sum_exp_chebyshev_angles}
\leanok
\end{proof}
...
\uses{lem:power_sum_invariance, thm:newton_identities, lem:esymm_invariant}
